...
function christmas_event_theme_styles() {
    // Deregister default style.css if it’s already registered
    wp_deregister_style('theme-style');
    
    // Enqueue the main.css file from the css folder
    wp_enqueue_style('main-style', get_template_directory_uri() . '/css/main.css', array(), '1.0', 'all');
}
add_action('wp_enqueue_scripts', 'christmas_event_theme_styles');

function enqueue_theme_scripts() {
    // Enqueue main.js
    wp_enqueue_script(
        'main-js', // Handle for the script
        get_template_directory_uri() . '/js/main.js', // Path to the script
        array('jquery'), // Dependencies (optional)
        null, // Version (optional, null to prevent caching issues)
